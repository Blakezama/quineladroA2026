...
def apostar(id_partido):
    ...
    # Bloqueo automático: 30 minutos antes del inicio (10 para Alemania vs Paraguay), o si el partido está en vivo o finalizado
    ahora = datetime.utcnow() - timedelta(hours=4)
    minutos_bloqueo = 10 if (partido.equipo_a == 'Alemania' and partido.equipo_b == 'Paraguay') or (partido.equipo_a == 'Paraguay' and partido.equipo_b == 'Alemania') else 30
    hora_limite = partido.fecha - timedelta(minutes=minutos_bloqueo)
...
def api_votar(id_partido):
    ...
    if ahora >= hora_limite:
        return jsonify({'error': f'⏰ Pronóstico cerrado. Las predicciones se bloquean {minutos_bloqueo} minutos antes del inicio del partido.'}), 403
